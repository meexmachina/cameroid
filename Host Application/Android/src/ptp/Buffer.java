...
/**
 * A Buffer handles marshaling/unmarshaling rules applicable to PTP over USB. That's a fairly standard sort of little-endian marshaling,
 * Unicode enabled. In this API, eight and sixteen bit values are passed in the least significant bits of integers, and are sign extended if
 * needed. Unsigned thirty-two and sixty-four bit values are not currently provided; they have the same bit patterns as signed values, but
 * applications must print them differently. One hundred twenty-eight bit integers are not currently supported. These buffers are not to be
 * used concurrently.
 * 
 * <p>
 * Strings representing times (<em>YYYYMMDDThhmmss[.s]{,Z,{+,-}hhmm}</em>) are not currently transformed to Java-oriented representations.
 * 
 */
public class Buffer
{
