...
public class deviceSelectionActivity extends Activity
{
	...
	private OnItemClickListener		m_DeviceClickListener	= new OnItemClickListener()
	{
		public void onItemClick(AdapterView<?> arg0, View view,
				int arg2, long arg3)
		{
			// Cancel discovery because it's
			// costly and we're about to
			// connect
			m_BtAdapter.cancelDiscovery();
			...
			// the
			// View
			String info = ((TextView) view).getText()
					.toString();
			try
			{
				// Attempt to extract a MAC
				// address
				String address = info
				...
				// and include the MAC
				// address
				Intent startMainPanel = new Intent(
						"afarsek.namespace.MAINPANELACTIVITY");
				startMainPanel.putExtra(EXTRA_DEVICE_ADDRESS,
						address);
			...
			{
				// Extraction failed, set
				// result and finish this
				// Activity
